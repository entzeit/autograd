...
seed = 826
print(f"Seed: {seed}")
X_train, Y_train, X_test, Y_test = DataGenerator([(lambda a, b: a * b)], binary=False, seed=seed).generate(normalize=True)

input_size = X_train[0].shape[0]
layers = [(10, Tensor.relu), (1, Tensor.identity)]
...
model.test(X_test, Y_test, tolerance=0.02, binary=False)